...
{phang2}
1. Creates the variables {bf:reward}, {bf:chosen_arm}, and {bf:batch} in Stata and initializes them so that they can be immediately used 
in combination with {bf:bbandits_update}.

{phang2}
2. If an exploration phase is defined, the initialize command will randomly with uniform probability assign the treatment arms for the exploration phase.

{phang2}
3. If the Sequential Arm Elimination (SAE) algorithm is specified, it returns a macro of a numlist of the available arms 
