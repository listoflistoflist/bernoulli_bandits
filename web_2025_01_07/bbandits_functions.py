...
def bernoulli_thompson_batched_clipping(
    ...
    """
    Bernoulli Thompson Sampling with batched play and exact lower-bounded clipping.
    """
...
###############################################################################
# to dos
# first calculate variance by arm
# calculate test statistic, confidence interval, estimate, variance (?)
#  Get all the same outcomes as in the conf int. function --> names etc everything should remain the same so it can be immediately transferred
...
def esfandiari_update(df,arms, active_arms, batch):
    ...
    if next_q * A < T_remain:
        ...
        # fill up batch variable
        start = df["batch"].isna().idxmax()   # first NaN position
        end = df["batch"].isna()[::-1].idxmax()
        # 2. fill the next batch_size_next rows with the batch number
        df.loc[start : end, "batch"] = next_batch
...
def check_and_run_greedy(
    ...
    if len(list_of_true_means) == len(list_of_standard_deviations):
        ...
        print("""Standard deviations not given or length of standard deviations do not match number of given means. 
Standard deviations for reward from normal are set to 1""")
