...
if !_rc {
    ...
    * String input → group by string labels
	  display as error "The assigned arm variable (input 2) is required to be numeric. It is recommended to number the arms from 0 up to arm k."
      exit   // <-- stops the program here
...
*/
	...
	*** Chose algorithm ***
	if  "`sae'" != ""{
		...
		*python: labels = pd.DataFrame({'code': range(len(labels)),'label': labels })
		*python: t = pd.merge(t, labels, left_on='active_arms',  right_on='code',  how='left').drop(columns='code') 
		* clear estimation results
		ereturn clear
		python: Matrix.store("e(active_arms)", t['active_arms'])
		*python: Matrix.setRowNames("e(active_arms)", t["label"].tolist())
	...
	/*
		...
		* If nothing is specified it should be the greedy algorithm
		display("Epsilon Greedy algorithm arm probabilities 1:")
		python: shares, chosen_arms, arm_values = epsilon_greedy_updating(t1["chosen_arm"].astype(int), t1["reward"], t1["batch"], pre["batch_size"],  epsilon = `epsilon')
		python: print(shares)
		python: Matrix.store("e(probabilities)", shares)
		python: Matrix.store("e(arm_labels)", arm_values)
...
*  for greedy and thompsons
	...
	*python: print(unique_pairs)
	*python: print(final["chosen_arm"])
	*python: print(final.head())
	*python: unique_pairs = unique_pairs.dropna(subset=['label_chosen_arm'])
